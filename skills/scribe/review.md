...
`ls -t .gubia/logs/*.out`. Order by mtime, never by the numeric prefix — the
engine's log sequence number (`log_seq`) is monotonic across relaunches, but
mtime stays the anchor as a defensive fallback for log sets left by an older
engine (which restarted the prefix at 1 on every relaunch) or copied in by
hand.
...
`loop_max_logs` most-recent iteration log sets, also ordered by modification
time for the same defensive reason — so there is no need to impose an
additional cap: read everything there is that is relevant.
